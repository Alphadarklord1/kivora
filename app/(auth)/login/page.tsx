...
export default function LoginPage() {
  ...
  const { t, isArabic } = useI18n({
    ...
    'Study system, not just sign-in': 'نظام دراسة وليس مجرد تسجيل دخول',
    'Get back to your workspace, planner, and saved outputs without losing the structure of your study flow.': 'ارجع إلى مساحة العمل والمخطط والمخرجات المحفوظة دون أن تفقد هيكل سير الدراسة.',
    'Guest mode keeps the app usable immediately, while sign-in unlocks synced plans, analytics, and linked providers.': 'وضع الضيف يبقي التطبيق قابلًا للاستخدام فورًا، بينما يفتح تسجيل الدخول المزامنة والتحليلات وربط المزوّدات.',
    ...
    <div className={styles.shell} dir={isArabic ? 'rtl' : 'ltr'}>
      <div className={styles.grid}>
        <section className={styles.panel}>
          ...
          <p className={styles.panelBody}>{t('Get back to your workspace, planner, and saved outputs without losing the structure of your study flow.')}</p>
          <div className={styles.proofGrid}>
            <div className={styles.proofCard}>
              <strong>{t('Use guest mode immediately')}</strong>
              <p>{t('Guest mode keeps the app usable immediately, while sign-in unlocks synced plans, analytics, and linked providers.')}</p>
            </div>
            <div className={styles.proofCard}>
              <strong>{t('Google and GitHub stay optional')}</strong>
              <p>{t('Use your browser, or continue as guest if you only need local study tools.')}</p>
            </div>
            <div className={styles.proofCard}>
              <strong>{t('Desktop and web share the same study workspace')}</strong>
              <p>{t('Access your study workspace, planner, and offline AI tools.')}</p>
            </div>
          </div>
