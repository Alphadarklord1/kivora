...
export const GLOBAL_TRANSLATIONS: Record<string, Partial<Record<SupportedLocale, string>>> = {
  ...
  'Optional model downloads are not ready yet': {
    ar: 'تنزيلات النماذج الاختيارية ليست جاهزة بعد',
    fr: 'Les téléchargements de modèles optionnels ne sont pas encore prêts',
  },
  'Keep using Mini until the larger local models are ready to install from inside Kivora.': {
    ar: 'استمر في استخدام Mini حتى تصبح النماذج المحلية الأكبر جاهزة للتثبيت من داخل Kivora.',
    fr: 'Continuez avec Mini jusqu’à ce que les modèles locaux plus grands soient prêts à installer dans Kivora.',
  },
  ...
  'Encryption Paused':  { ar: 'التشفير معطل',  fr: 'Chiffrement suspendu' },
  'Encryption disabled for this release': { ar: 'التشفير معطل في هذا الإصدار', fr: 'Chiffrement désactivé pour cette version' },
  'Local vault password prompts are turned off until encryption returns in a later release.':
    { ar: 'تم إيقاف مطالبات كلمة مرور الخزنة المحلية حتى يعود التشفير في إصدار لاحق.',
      fr: 'Les invites de mot de passe du coffre local sont désactivées jusqu\'au retour du chiffrement dans une version ultérieure.' },
