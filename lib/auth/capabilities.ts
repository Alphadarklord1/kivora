...
function isGuestModeEnabledForCapabilities(): boolean {
  ...
  if (
    ...
    readBooleanEnv(process.env.KIVORA_DESKTOP_ONLY || process.env.STUDYPILOT_DESKTOP_ONLY)
  ) {
    return true;
  }
...
export function getDesktopAuthPort(): number {
  return parsePort(process.env.KIVORA_DESKTOP_AUTH_PORT || process.env.STUDYPILOT_DESKTOP_AUTH_PORT);
}
...
export function getAuthCapabilities() {
  ...
  const explicitOauthDisabled = (process.env.KIVORA_OAUTH_DISABLED || process.env.STUDYPILOT_OAUTH_DISABLED) === '1';
  ...
  const oauthDisabledReason = authDisabled
    ? authDisabledReason
    : (process.env.KIVORA_OAUTH_DISABLED_REASON || process.env.STUDYPILOT_OAUTH_DISABLED_REASON || null);
  const desktopAuthPort =
    (process.env.KIVORA_DESKTOP_ONLY || process.env.STUDYPILOT_DESKTOP_ONLY) === '1'
      ? getDesktopAuthPort()
      : null;
